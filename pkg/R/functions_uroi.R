...
#' @param k_vec Vector of number of clusters
								  ...
								  colormode="Color")
{
	...
	if(colormode=="Color"){
		...
		stop("Wrong colormode '",colormode,"' specified \n")
	}
	...
	for(n_pc in n_pc_vec){
		...
		if(colormode=="Grayscale"){
			...
			u <- matrix(svd_images$u, ncol=n_pc*3)
		}
	...
	settings <- expand.grid(k_vec, n_pc_vec)
	colnames(settings) <- c("K", "npc")
	...
	if(save_results){
		save(cluster_list,
			  settings,
			  file=paste0(path_base,name_of_analysis,"/clusters.RData"))
	}

	output <- list(cluster_list=cluster_list,
						settings=settings,
